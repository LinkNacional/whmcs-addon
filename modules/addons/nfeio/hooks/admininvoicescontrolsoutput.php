...
if ($_REQUEST['gnfe_create']) {
    if ($nfe_for_invoice['status'] !== (string) 'Created' && $nfe_for_invoice['status'] !== (string) 'Issued' && $nfe_for_invoice['status'] !== (string) 'Waiting') {
        ...
        if ($queue !== 'success') {
            nfeio_log('nfeio', 'admininvoicecontorloutput - gnfe_create',$vars['invoiceid'], $queue, 'ERROR', '');
            header_remove();
            header('Location: ' . $nfeioWhmcsAdminUrl . 'invoices.php?action=edit&id=' . $vars['invoiceid'] . '&gnfe_error=Erro ao criar nota fiscal: ' . $queue);

            exit;
        }
        if ($queue === 'success') {
            nfeio_log('nfeio', 'admininvoicecontorloutput - gnfe_create',$vars['invoiceid'], $queue, 'OK', '');
            ...
            header_remove();
            header('Location: ' . $nfeioWhmcsAdminUrl . 'invoices.php?action=edit&id=' . $vars['invoiceid'] . '&gnfe_message=' . base64_encode(urlencode($message)));
...
if ($_REQUEST['gnfe_cancel']) {
    ...
    if (!$delete_nfe->message) {
        nfeio_log('nfeio', 'admininvoicecontorloutput - gnfe_cancel',$nfe->nfe_id, $delete_nfe, 'OK', '');
...
if ($_REQUEST['gnfe_email']) {
    ...
    if ($gnfe_email->message) {
        nfeio_log('nfeio', 'admininvoicecontorloutput - gnfe_email',$_REQUEST['gnfe_email'], $gnfe_email, 'ERROR', '');
...
echo '<button ' . $disabled['a'] . ' style="margin-right: 4px;" onclick="location.href=`' . $nfeioWhmcsAdminUrl . 'invoices.php?action=edit&id=' . $vars['invoiceid'] . '&gnfe_create=yes`" class="btn btn-primary" id="gnfe_generate" title="Emitir Nota Fiscal">Emitir NFE</button>';
echo '<button ' . $disabled['b'] . ' style="margin-right: 4px;" onclick="location.href=`' . $nfeioWhmcsAdminUrl . 'invoices.php?action=edit&id=' . $vars['invoiceid'] . '&gnfe_open=' . $vars['invoiceid'] . '`" class="btn btn-success" id="gnfe_view" title="Ver Nota Fiscal">Visualizar NFE</button>';
echo '<button ' . $disabled['c'] . ' style="margin-right: 4px;" onclick="location.href=`' . $nfeioWhmcsAdminUrl . 'invoices.php?action=edit&id=' . $vars['invoiceid'] . '&gnfe_cancel=' . $nfe_for_invoice['nfe_id'] . '`" class="btn btn-danger" id="gnfe_cancel" title="Cancelar Nota Fiscal">Cancelar NFE</button>';
echo '<button ' . $disabled['d'] . ' onclick="location.href=`' . $nfeioWhmcsAdminUrl . 'invoices.php?action=edit&id=' . $vars['invoiceid'] . '&gnfe_email=' . $nfe_for_invoice['nfe_id'] . '`" class="btn btn-primary" id="gnfe_email" title="Enviar Nota Fiscal por Email">Enviar Email</button>';
echo '<div>';
