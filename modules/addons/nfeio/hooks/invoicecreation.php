...
if ($issueInvoiceCondition === 'quando a fatura é gerada') {
    nfeio_log('nfeio', 'quando a fatura é gerada invoicecreation', $issueInvoiceCondition , '', '', '');
    ...
    if ((float) $invoice['total'] > (float) '0.00' and $invoice['status'] != 'Draft') {
        ...
        if (!$nfe_for_invoice['id']) {
            ...
            if ($queue !== 'success') {
                nfeio_log('nfeio', 'invoicecreation', $vars['invoiceid'], $queue, 'ERROR', '');
                ...
                nfeio_log('nfeio', 'invoicecreation', $vars['invoiceid'], $queue, 'OK', '');
            }
        }
    }
} elseif ($issueInvoiceCondition === 'quando a fatura é paga') {
    nfeio_log('nfeio', 'quando a fatura é paga invoicecreation', '', '', '', '');

    return;
} else {
    nfeio_log('nfeio', 'seguir configuração do módulo nfe.io invoicecreation', '', '', '', '');
    $params = nfeio_get_setting();
    if (stripos($params['issue_note_default_cond'], 'Gerada') && (string) $vars['status'] != 'Draft' && (!$params['issue_note_after_days'] || 0 == $params['issue_note_after_days'])) {
        $invoice = localAPI('GetInvoice', ['invoiceid' => $vars['invoiceid']], false);
