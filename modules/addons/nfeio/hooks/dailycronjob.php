...
$params = nfeio_get_setting('issue_note_after_days');

$data = getTodaysDate(false);
$currentDate = toMySQLDate($data);

if (isset($issueNoteAfterDays) && (int)$issueNoteAfterDays > 0) {
    foreach (Capsule::table('tblinvoices')->whereBetween('date', [$params['initial_date'], $currentDate])->where('status', '=', 'Paid')->get(['id', 'userid', 'datepaid', 'total']) as $invoices) {
